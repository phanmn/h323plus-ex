...
ifeq ($(OS), Linux)
	...
	WRAPPER_INCLUDES = -I$(ERLANG_INCLUDE) -I$(PTLIB_DIR)/include -I$(H323PLUS_DIR)/include $(OPENSSL_CFLAGS)
    ...
    SYS_LIBS = $(OPENSSL_LIBS) -lpthread
    LDFLAGS = -shared
else
    # macOS
    # Include paths for wrapper compilation
...
ifeq ($(OS), Linux)
  NIF_LINK_CMD := $(CXX) $(CXXFLAGS) $(NIF_INCLUDES) -shared -o $@ $^ $(H323_LIB) $(PTLIB_LIB) $(SYS_LIBS)
else  # macOS
